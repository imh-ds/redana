"""Immutable F5 explicit-quadratic residual-null repair execution."""
...
from research.gate0.residuals import (
    ...
    cross_fitted_pair_residuals,  # noqa: F401 - negative test spy target.
)
...
class F5QuadraticRepairConfig:
    """Frozen dimensions for the F5 raw-plus-square residual repair run."""
    ...
    def gate0_config(self) -> Gate0Config:
        ...
        return Gate0Config(
            ComputationalProfile(
                "f5-quadratic-repair", self.rows, self.rows, 1, self.permutations
            ),
...
@dataclass(frozen=True)
class F5QuadraticRepairRecord:
    """One attempted F5 explicit-quadratic residual dependence calculation."""
...
def run_f5_quadratic_repair(
    output_dir: Path, run_id: str, config: F5QuadraticRepairConfig
) -> pd.DataFrame:
    """Attempt every frozen F5 cell and retain its primary evidence atomically."""
...
def _record(
    ...
    return F5QuadraticRepairRecord(
        fixture_id="F5",
        left="X1",
        right="X2",
        phase=_PHASE,
...
def _records_frame(records: list[F5QuadraticRepairRecord]) -> pd.DataFrame:
