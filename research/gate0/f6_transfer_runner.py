"""Immutable F6 residual-null transfer execution."""
...
class F6TransferConfig:
    """Frozen dimensions and residualization settings for the F6 transfer run."""
    ...
    def gate0_config(self) -> Gate0Config:
        ...
        return Gate0Config(
            ComputationalProfile("f6-transfer", self.rows, self.rows, 1, self.permutations),
...
@dataclass(frozen=True)
class F6TransferRecord:
    """One attempted F6 residual dependence calculation."""
...
def run_f6_transfer(
    output_dir: Path, run_id: str, config: F6TransferConfig
) -> pd.DataFrame:
    """Attempt every frozen F6 cell and retain its primary evidence atomically."""
...
def _run_cell(
    ...
    try:
        with warnings.catch_warnings(record=True) as caught:
            ...
            frame = generate_fixture("F6", config.rows, fixture_seed)
...
def _records_frame(records: list[F6TransferRecord]) -> pd.DataFrame:
