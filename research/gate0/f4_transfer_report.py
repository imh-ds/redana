"""Hash-pinned evidence reports for the frozen F4 residual-null transfer."""
...
_PHASE = "f4-linear-null-transfer"
_FIXTURE = "F4"
_PAIR = ("X1", "X3")
_NAMESPACE = "batch-f4-linear-null-transfer"
_REQUIRED_COLUMNS = {
    "fixture_id",
    "left",
    "right",
...
def write_f4_transfer_report(
    ...
    """Write immutable F4 transfer evidence against the copied reference boundary."""

    _validate_f4_records(records, output_dir, run_id)
    ...
    summary.to_csv(output_dir / "f4-transfer-summary.csv", index=False)
    _plot_batch_classes(summary, output_dir / "plots" / "f4-batch-classifications.png")
    manifest = {
        "run_id": run_id,
        "phase": _PHASE,
        "fixture_id": _FIXTURE,
        ...
        "summary_path": "f4-transfer-summary.csv",
        "plots": ["plots/f4-batch-classifications.png"],
    ...
    memo_path = output_dir / "f4-transfer-memo.md"
...
def _validate_f4_records(records: pd.DataFrame, output_dir: Path, run_id: str) -> None:
    missing = _REQUIRED_COLUMNS.difference(records.columns)
    if missing:
        raise ValueError(f"F4 report requires record columns: {sorted(missing)}")
    if records.empty or records["run_id"].isna().any() or not records["run_id"].eq(run_id).all():
        raise ValueError("F4 record run ID does not match report run_id")
    if not records["fixture_id"].eq(_FIXTURE).all() or not records["left"].eq(_PAIR[0]).all() or not records[
        "right"
    ].eq(_PAIR[1]).all():
        raise ValueError("F4 report requires only fixture F4 and pair X1/X3")
    if not records["seed_namespace"].eq(_NAMESPACE).all():
        raise ValueError("F4 report requires the frozen seed namespace")
    if not records["phase"].eq(_PHASE).all():
        raise ValueError("F4 report requires only F4 residual-null transfer records")
    for relative_path in records["residual_samples_path"]:
        if not isinstance(relative_path, str) or not relative_path:
            raise ValueError("F4 report requires every residual sample path")
        path = output_dir / relative_path
        if not path.is_file():
            raise ValueError(f"F4 report requires retained residual sample: {relative_path}")
...
def _runner_provenance(
    ...
    if not records_path.is_file() or not input_path.is_file():
        raise ValueError("F4 report requires retained records.csv and manifest-input.json")
    try:
        runner_input = json.loads(input_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError("F4 report requires valid manifest-input.json") from error
    if not isinstance(runner_input, dict):
        raise TypeError("F4 report requires object manifest-input.json")
    if runner_input.get("fixture_id") not in (None, _FIXTURE):
        raise ValueError("F4 runner provenance has the wrong fixture")
    if runner_input.get("pair") not in (None, list(_PAIR)):
        raise ValueError("F4 runner provenance has the wrong pair")
    if runner_input.get("run_id") not in (None, records["run_id"].iloc[0]):
        raise ValueError("F4 runner provenance has the wrong run ID")
    if runner_input.get("seed_namespace") != _NAMESPACE:
        raise ValueError("F4 runner provenance has the wrong seed namespace")
    return {
...
def _plot_batch_classes(summary: pd.DataFrame, path: Path) -> None:
    ...
    axis.set_title("F4 residual-null batch classifications")
    figure.tight_layout()
...
def _memo(
    ...
    return "\n".join(
        [
            "# F4 linear residual-null transfer memo",
            "",
            f"Run ID: `{run_id}`",
            ...
            "",
            f"- F4 records SHA-256: `{provenance['records_sha256']}`",
