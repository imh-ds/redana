"""Immutable F3 nonlinear direct-edge detection execution."""
...
class F3NonlinearDirectEdgeDetectionRecord:
    """One attempted F3 nonlinear direct-edge detection calculation, retaining failures."""
...
def run_f3_nonlinear_direct_edge_detection(
    output_dir: Path, run_id: str, config: F4LinkConfig
) -> pd.DataFrame:
    """Attempt every frozen F3 cell and retain its primary evidence atomically."""
...
def _gate0_config(config: F4LinkConfig) -> Gate0Config:
    return Gate0Config(
        ComputationalProfile(
            "f3-nonlinear-direct-edge-detection", config.rows, config.rows, 1, config.permutations
        ),
...
) -> F3NonlinearDirectEdgeDetectionRecord:
...
def _record(
    ...
    return F3NonlinearDirectEdgeDetectionRecord(
...
def _records_frame(records: list[F3NonlinearDirectEdgeDetectionRecord]) -> pd.DataFrame:
