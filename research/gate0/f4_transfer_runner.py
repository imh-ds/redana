"""Immutable F4 linear residual-null transfer execution."""
...
class F4TransferConfig:
    """Frozen dimensions and residualization settings for the F4 transfer run."""
    ...
    def gate0_config(self) -> Gate0Config:
        ...
        return Gate0Config(
            ComputationalProfile("f4-transfer", self.rows, self.rows, 1, self.permutations),
...
@dataclass(frozen=True)
class F4TransferRecord:
    """One attempted F4 residual dependence calculation."""
...
def run_f4_transfer(
    output_dir: Path, run_id: str, config: F4TransferConfig
) -> pd.DataFrame:
    """Attempt every frozen F4 cell and retain its primary evidence atomically."""
...
def _run_cell(
    ...
    try:
        with warnings.catch_warnings(record=True) as caught:
            ...
            frame = generate_fixture("F4", config.rows, fixture_seed)
...
def _records_frame(records: list[F4TransferRecord]) -> pd.DataFrame:
