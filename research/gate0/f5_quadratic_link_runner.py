"""Immutable execution for the F5 quadratic-residual-link alternative."""
...
from research.gate0.residuals import cross_fitted_pair_quadratic_residuals
...
class F5QuadraticLinkRecord:
    """One attempted Candidate 1 quadratic-link calculation, retaining failures."""
...
def generate_f5_quadratic_link_fixture(rows: int, seed: int) -> pd.DataFrame:
    """Generate the fixed Candidate 1 DGP and standardize once after construction."""
    ...
    x1 = parent + e1
    x2 = parent + FIXTURE_COEFFICIENT * (e1**2 - 1) + e2
    frame = pd.DataFrame({"X1": x1, "X2": x2, "X3": e3, "X4": e4, "X5": e5, "X6": e6})
    return (frame - frame.mean()) / frame.std(ddof=0)


def run_f5_quadratic_link(output_dir: Path, run_id: str, config: F4LinkConfig) -> pd.DataFrame:
...
def _gate0_config(config: F4LinkConfig) -> Gate0Config:
    return Gate0Config(
        ComputationalProfile(
            "f5-quadratic-residual-link", config.rows, config.rows, 1, config.permutations
        ),
...
def _run_cell(
    ...
    try:
        with warnings.catch_warnings(record=True) as caught:
            ...
            frame = generate_f5_quadratic_link_fixture(config.rows, fixture_seed)
...
def _records_frame(records: list[F5QuadraticLinkRecord]) -> pd.DataFrame:
