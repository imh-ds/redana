"""Immutable F5 residual-null transfer execution."""
...
class F5TransferConfig:
    """Frozen dimensions and residualization settings for the F5 transfer run."""
    ...
    def gate0_config(self) -> Gate0Config:
        ...
        return Gate0Config(
            ComputationalProfile("f5-transfer", self.rows, self.rows, 1, self.permutations),
...
@dataclass(frozen=True)
class F5TransferRecord:
    """One attempted F5 residual dependence calculation."""
...
def _records_frame(records: list[F5TransferRecord]) -> pd.DataFrame:
