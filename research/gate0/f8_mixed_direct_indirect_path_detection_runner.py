"""Immutable F8 mixed direct-and-indirect path detection execution."""
...
class F8MixedDirectIndirectPathDetectionRecord:
    """One attempted F8 mixed direct-and-indirect path detection calculation, retaining failures."""
...
def run_f8_mixed_direct_indirect_path_detection(
    output_dir: Path, run_id: str, config: F4LinkConfig
) -> pd.DataFrame:
    """Attempt every frozen F8 cell and retain its primary evidence atomically."""
...
def _gate0_config(config: F4LinkConfig) -> Gate0Config:
    return Gate0Config(
        ComputationalProfile(
            "f8-mixed-direct-indirect-path-detection",
            config.rows,
            config.rows,
            1,
            config.permutations,
...
) -> F8MixedDirectIndirectPathDetectionRecord:
...
def _record(
    ...
    return F8MixedDirectIndirectPathDetectionRecord(
...
def _records_frame(
    records: list[F8MixedDirectIndirectPathDetectionRecord],
) -> pd.DataFrame:
