"""Immutable F2 linear direct-edge detection execution."""
...
class F2LinearDirectEdgeDetectionRecord:
    """One attempted F2 linear direct-edge detection calculation, retaining failures."""
...
def run_f2_linear_direct_edge_detection(
    output_dir: Path, run_id: str, config: F4LinkConfig
) -> pd.DataFrame:
    """Attempt every frozen F2 cell and retain its primary evidence atomically."""
...
def _gate0_config(config: F4LinkConfig) -> Gate0Config:
    return Gate0Config(
        ComputationalProfile(
            "f2-linear-direct-edge-detection", config.rows, config.rows, 1, config.permutations
        ),
...
) -> F2LinearDirectEdgeDetectionRecord:
...
def _record(
    ...
    return F2LinearDirectEdgeDetectionRecord(
...
def _records_frame(records: list[F2LinearDirectEdgeDetectionRecord]) -> pd.DataFrame:
