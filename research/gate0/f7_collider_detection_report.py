"""Hash-pinned evidence reports for the F7 collider detection study."""
...
def write_f7_collider_detection_report(
    ...
    summary.to_csv(output_dir / "f7-collider-detection-summary.csv", index=False)
    _plot_detection_batches(summary, output_dir / "plots" / "f7-collider-detections.png")
    manifest = {
        "run_id": run_id,
        "phase": _PHASE,
        "fixture_id": _FIXTURE,
        ...
        "summary_path": "f7-collider-detection-summary.csv",
        "plots": ["plots/f7-collider-detections.png"],
    ...
    memo_path = output_dir / "f7-collider-detection-memo.md"
...
def _validate_records(records: pd.DataFrame, output_dir: Path, run_id: str) -> None:
    ...
    if missing:
        raise ValueError(f"F7 collider-detection report requires record columns: {sorted(missing)}")
    if records.empty or records["run_id"].isna().any() or not records["run_id"].eq(run_id).all():
        raise ValueError("F7 collider-detection record run ID does not match report run_id")
    if not records["fixture_id"].eq(_FIXTURE).all():
        raise ValueError("F7 collider-detection report requires the F7 fixture")
    if not records["left"].eq(_PAIR[0]).all() or not records["right"].eq(_PAIR[1]).all():
        raise ValueError("F7 collider-detection report requires pair X1/X2")
    if not records["phase"].eq(_PHASE).all():
        raise ValueError("F7 collider-detection report requires the frozen phase")
    if not records["seed_namespace"].eq(_NAMESPACE).all():
        raise ValueError("F7 collider-detection report requires the frozen seed namespace")
    for row in records.itertuples(index=False):
        ...
        for name, label in (
            ...
            if not isinstance(name, str) or not name or not (output_dir / name).is_file():
                raise ValueError(f"F7 collider-detection report requires retained {label}")
...
def _runner_provenance(
    ...
    if not records_path.is_file() or not input_path.is_file():
        raise ValueError("F7 collider-detection report requires retained records.csv and manifest-input.json")
    try:
        payload = json.loads(input_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError("F7 collider-detection report requires valid manifest-input.json") from error
    if not isinstance(payload, dict):
        raise TypeError("F7 collider-detection report requires object manifest-input.json")
    expected = {
        "fixture_id": _FIXTURE, "pair": list(_PAIR), "phase": _PHASE,
        "run_id": records["run_id"].iloc[0], "seed_namespace": _NAMESPACE,
    }
    if any(payload.get(key) != value for key, value in expected.items()):
        raise ValueError("F7 collider-detection runner provenance does not match the frozen scenario")
    return {
...
def _plot_detection_batches(summary: pd.DataFrame, path: Path) -> None:
    ...
    axis.set_title("F7 collider batch detections")
    figure.tight_layout()
...
def _memo(
    ...
    return "\n".join([
        "# F7 collider detection memo", "", f"Run ID: `{run_id}`", f"Terminal outcome: **{outcome}**", "",
        "## Copied reference calibration", "", f"- Calibration manifest SHA-256: `{calibration['manifest_sha256']}`",
        ...
        "- Fixture and pair: `F7`, `(X1, X2)`", "- Adjustment variables: `(X3, X4, X5, X6)`",
        (
            "- X3 is a collider (`X3 = 0.7*X1 + 0.7*X2 + e3`); adjusting for it induces real "
            "dependence between X1 and X2, so detection here is the mathematically correct outcome."
        ),
