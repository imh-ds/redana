"""Hash-pinned evidence reports for the F3 nonlinear direct-edge detection study."""
...
_PHASE = "f3-nonlinear-direct-edge-detection"
_FIXTURE = "F3"
_PAIR = ("X1", "X2")
_NAMESPACE = "batch-f3-nonlinear-direct-edge-detection"
_REQUIRED_COLUMNS = {
...
def write_f3_nonlinear_direct_edge_detection_report(
    ...
    summary.to_csv(output_dir / "f3-nonlinear-direct-edge-detection-summary.csv", index=False)
    _plot_detection_batches(
        summary, output_dir / "plots" / "f3-nonlinear-direct-edge-detections.png"
    )
    manifest = {
        "run_id": run_id,
        "phase": _PHASE,
        ...
        "summary_path": "f3-nonlinear-direct-edge-detection-summary.csv",
        "plots": ["plots/f3-nonlinear-direct-edge-detections.png"],
    ...
    memo_path = output_dir / "f3-nonlinear-direct-edge-detection-memo.md"
...
def _validate_records(records: pd.DataFrame, output_dir: Path, run_id: str) -> None:
    ...
    if missing:
        raise ValueError(
            f"F3 nonlinear direct-edge detection report requires record columns: {sorted(missing)}"
        )
    if records.empty or records["run_id"].isna().any() or not records["run_id"].eq(run_id).all():
        raise ValueError("F3 nonlinear direct-edge detection record run ID does not match report run_id")
    if not records["fixture_id"].eq(_FIXTURE).all():
        raise ValueError("F3 nonlinear direct-edge detection report requires the F3 fixture")
    if not records["left"].eq(_PAIR[0]).all() or not records["right"].eq(_PAIR[1]).all():
        raise ValueError("F3 nonlinear direct-edge detection report requires pair X1/X2")
    if not records["phase"].eq(_PHASE).all():
        raise ValueError("F3 nonlinear direct-edge detection report requires the frozen phase")
    if not records["seed_namespace"].eq(_NAMESPACE).all():
        raise ValueError("F3 nonlinear direct-edge detection report requires the frozen seed namespace")
    for row in records.itertuples(index=False):
        ...
        for name, label in (
            ...
            if not isinstance(name, str) or not name or not (output_dir / name).is_file():
                raise ValueError(
                    f"F3 nonlinear direct-edge detection report requires retained {label}"
                )
...
def _runner_provenance(
    ...
    if not records_path.is_file() or not input_path.is_file():
        raise ValueError(
            "F3 nonlinear direct-edge detection report requires retained records.csv and manifest-input.json"
        )
    try:
        payload = json.loads(input_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError(
            "F3 nonlinear direct-edge detection report requires valid manifest-input.json"
        ) from error
    if not isinstance(payload, dict):
        raise TypeError("F3 nonlinear direct-edge detection report requires object manifest-input.json")
    expected = {
    ...
    if any(payload.get(key) != value for key, value in expected.items()):
        raise ValueError(
            "F3 nonlinear direct-edge detection runner provenance does not match the frozen scenario"
        )
    return {
...
def _plot_detection_batches(summary: pd.DataFrame, path: Path) -> None:
    ...
    axis.set_title("F3 nonlinear direct-edge batch detections")
    figure.tight_layout()
...
def _memo(
    ...
    return "\n".join([
        "# F3 nonlinear direct-edge detection memo", "", f"Run ID: `{run_id}`",
        ...
        "- Fixture and pair: `F3`, `(X1, X2)`", "- Adjustment variables: `(X3, X4, X5, X6)` -- signal-free by construction",
        (
            "- X2 is a direct nonlinear function of X1 (`X2 = 0.7*(X1^2-1)+e2`) with no common "
            "cause, mediator, or collider; this isolates raw nonlinear-edge detection from F5's "
            "adjustment-transfer problem."
        ),
