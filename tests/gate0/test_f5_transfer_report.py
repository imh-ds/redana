"""Evidence-report contracts for the frozen F5 residual-null transfer."""
...
from research.gate0.f5_transfer_report import write_f5_transfer_report
...
def _retain_plot_path_without_a_graphics_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("research.gate0.f5_transfer_report._plot_batch_classes", write_plot)
...
def _write_runner_inputs(path: Path, records: pd.DataFrame) -> None:
    ...
    (path / "manifest-input.json").write_text(
        json.dumps({"config": {}, "seed_namespace": "batch-f5-null-transfer"}), encoding="utf-8"
    )
...
def _write_report(tmp_path: Path, records: pd.DataFrame) -> Path:
    ...
    return write_f5_transfer_report(
        records, output, _RUN_ID, _ready_calibration(tmp_path / "calibration"), _TransferConfig()
    )


def test_complete_f5_frame_passes_and_copies_frozen_boundary(tmp_path: Path) -> None:
    """A complete F5-only frame applies the unchanged confirmation policy."""
    ...
    assert (tmp_path / "transfer" / "f5-transfer-summary.csv").is_file()
    assert (tmp_path / "transfer" / "plots" / "f5-batch-classifications.png").is_file()
...
def test_tampered_calibration_records_are_refused_before_report_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="SHA-256"):
        write_f5_transfer_report(records, output, _RUN_ID, calibration, _TransferConfig())
...
def test_missing_residual_sample_path_is_rejected(tmp_path: Path) -> None:
    """Every successful F5 record must point to a retained residual sample."""
