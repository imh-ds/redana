"""Evidence-report contracts for the F3 nonlinear direct-edge detection study."""
...
def _retain_plot_path_without_a_graphics_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "research.gate0.f3_nonlinear_direct_edge_detection_report._plot_detection_batches",
        write_plot,
    )
            ...
            rows.append(
                {
                    ...
                    "left": "X1",
                    "right": "X2",
                    "phase": "f3-nonlinear-direct-edge-detection",
                    "batch": batch,
                    ...
                    "seed_namespace": "batch-f3-nonlinear-direct-edge-detection",
                    "run_id": _RUN_ID,
...
def _write_report(tmp_path: Path, records: pd.DataFrame) -> Path:
    ...
    return write_f3_nonlinear_direct_edge_detection_report(
...
def test_complete_detection_evidence_passes_and_copies_boundary(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "detection" / "f3-nonlinear-direct-edge-detection-summary.csv").is_file()
    assert (tmp_path / "detection" / "plots" / "f3-nonlinear-direct-edge-detections.png").is_file()
