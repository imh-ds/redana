"""Command-line contracts for the frozen F2 linear direct-edge detection study."""
...
def test_cli_refuses_altered_calibration_provenance_before_creating_output(
    ...
    from scripts import run_f2_linear_direct_edge_detection as cli
...
def test_cli_uses_only_output_and_run_id_and_refuses_reuse(
    ...
    monkeypatch.setattr(cli, "run_f2_linear_direct_edge_detection", fake_run)
    monkeypatch.setattr(cli, "write_f2_linear_direct_edge_detection_report", fake_report)
    output = tmp_path / "output"
