"""Evidence-report contracts for the F8 mixed direct-and-indirect path detection study."""
...
def _retain_plot_path_without_a_graphics_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "research.gate0.f8_mixed_direct_indirect_path_detection_report._plot_detection_batches",
        write_plot,
    )
            ...
            rows.append(
                {
                    ...
                    "left": "X1",
                    "right": "X2",
                    "phase": "f8-mixed-direct-indirect-path-detection",
                    "batch": batch,
                    ...
                    "seed_namespace": "batch-f8-mixed-direct-indirect-path-detection",
                    "run_id": _RUN_ID,
...
def _write_report(tmp_path: Path, records: pd.DataFrame) -> Path:
    ...
    return write_f8_mixed_direct_indirect_path_detection_report(
...
def test_complete_detection_evidence_passes_and_copies_boundary(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "detection" / "f8-mixed-direct-indirect-path-detection-summary.csv").is_file()
    assert (tmp_path / "detection" / "plots" / "f8-mixed-direct-indirect-path-detections.png").is_file()
