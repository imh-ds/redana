...
from research.gate0 import f3_nonlinear_direct_edge_detection_runner
...
def test_runner_generates_only_f3_and_retains_every_successful_cell(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    calls: list[tuple[str, int]] = []
    original = f3_nonlinear_direct_edge_detection_runner.generate_fixture

    def spy(fixture_id: str, rows: int, seed: int) -> pd.DataFrame:
        calls.append((fixture_id, rows))
        return original(fixture_id, rows, seed)

    monkeypatch.setattr(f3_nonlinear_direct_edge_detection_runner, "generate_fixture", spy)

    frame = run_f3_nonlinear_direct_edge_detection(tmp_path, "unit", _small_config())

    assert calls == [("F3", 100)] * 6
    assert len(frame) == 6
    ...
    assert frame["seed_namespace"].eq("batch-f3-nonlinear-direct-edge-detection").all()
    assert frame["phase"].eq("f3-nonlinear-direct-edge-detection").all()
...
def test_runner_retains_one_residualization_failure_and_continues(
    ...
    original = f3_nonlinear_direct_edge_detection_runner.cross_fitted_pair_residuals
    calls = 0
    ...
    monkeypatch.setattr(
        f3_nonlinear_direct_edge_detection_runner, "cross_fitted_pair_residuals", fail_once
    )

    frame = run_f3_nonlinear_direct_edge_detection(tmp_path, "failure", _small_config())
...
def test_runner_refuses_nonempty_output_without_modifying_it(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileExistsError, match="initialized"):
        run_f3_nonlinear_direct_edge_detection(output_dir, "taken", _small_config())
...
def test_runner_rejects_blank_run_id(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="run_id must be non-empty"):
        run_f3_nonlinear_direct_edge_detection(tmp_path / "out", "  ", _small_config())
