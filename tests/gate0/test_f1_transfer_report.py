"""Evidence-report contracts for the frozen F1 independence null transfer."""
...
from research.gate0.f1_transfer_report import write_f1_transfer_report
...
def _retain_plot_path_without_a_graphics_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("research.gate0.f1_transfer_report._plot_batch_classes", write_plot)
            ...
            rows.append(
                {
                    ...
                    "seed_namespace": "batch-f1-null-transfer",
                    "run_id": _RUN_ID,
                    "warnings": None,
                    "exception_text": None,
                }
...
def _write_runner_inputs(path: Path, records: pd.DataFrame) -> None:
    ...
    (path / "manifest-input.json").write_text(
        json.dumps({"config": {}, "seed_namespace": "batch-f1-null-transfer"}), encoding="utf-8"
    )
    for relative_path in records["residual_samples_path"]:
        target = path / relative_path
        target.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame({"X1": [0.0], "X2": [0.0]}).to_csv(target, index=False)


def _write_report(tmp_path: Path, records: pd.DataFrame) -> Path:
    output = tmp_path / "transfer"
    _write_runner_inputs(output, records)
    return write_f1_transfer_report(
        records, output, _RUN_ID, _ready_calibration(tmp_path / "calibration"), _TransferConfig()
    )


def test_complete_f1_frame_passes_and_copies_frozen_boundary(tmp_path: Path) -> None:
    """A complete F1-only frame applies the unchanged confirmation policy."""
    ...
    assert (tmp_path / "transfer" / "f1-transfer-summary.csv").is_file()
    assert (tmp_path / "transfer" / "plots" / "f1-batch-classifications.png").is_file()
...
def test_tampered_calibration_records_are_refused_before_report_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="SHA-256"):
        write_f1_transfer_report(records, output, _RUN_ID, calibration, _TransferConfig())
...
def test_missing_residual_sample_path_is_rejected(tmp_path: Path) -> None:
    """Every successful F1 record must point to a retained residual sample."""
...
@pytest.mark.parametrize(
    ...
    [
        ("fixture_id", "F2", "fixture F1"),
        ("left", "X3", "fixture F1"),
        ("phase", "wrong", "F1 residual-null transfer records"),
...
def test_wrong_transfer_identity_is_rejected(
    ...
    """An identity mismatch must not be summarized as F1 transfer evidence."""
