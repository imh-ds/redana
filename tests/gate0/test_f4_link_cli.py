"""Command-line contracts for the frozen F4 residual-link alternative."""
...
def _approved_f4_null(path: Path) -> None:
    path.mkdir(parents=True)
    records = path / "records.csv"
    runner_input = path / "manifest-input.json"
    records.write_text("f4 null\n", encoding="utf-8")
    runner_input.write_text("{}\n", encoding="utf-8")
    (path / "manifest.json").write_text(
        json.dumps(
            {
...
@pytest.mark.parametrize("parent", ["calibration", "f4_null"])
def test_cli_refuses_altered_parent_provenance_before_creating_output(
    tmp_path: Path, monkeypatch, capsys, parent: str
) -> None:
    from scripts import run_f4_residual_link_alternative as cli

    calibration = tmp_path / "calibration"
    f4_null = tmp_path / "f4-null"
    _approved_calibration(calibration, cli._FROZEN_BOUNDARY)
    _approved_f4_null(f4_null)
    monkeypatch.setattr(cli, "_CALIBRATION_DIR", calibration)
    monkeypatch.setattr(cli, "_F4_NULL_DIR", f4_null)
    (calibration if parent == "calibration" else f4_null).joinpath("records.csv").write_text(
        "altered\n", encoding="utf-8"
    )
...
def test_cli_uses_only_output_and_run_id_and_refuses_reuse(
    ...
    def fake_report(
        records: pd.DataFrame, output_dir: Path, run_id: str, parent: Path, calibration_dir: Path, config: object
    ) -> Path:
        assert len(records) == 1
        assert parent == f4_null
    ...
    monkeypatch.setattr(cli, "run_f4_link", fake_run)
    monkeypatch.setattr(cli, "write_f4_link_report", fake_report)
    output = tmp_path / "output"
