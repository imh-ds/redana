...
def test_f5_cli_refuses_tampered_calibration_before_creating_output(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    """The narrow CLI validates its frozen calibration before initializing a run directory."""

    from scripts import run_f5_residual_null_transfer as cli
    ...
    assert "F5 RESIDUAL-NULL TRANSFER REFUSED [unit]: SHA-256 mismatch" in capsys.readouterr().err


def test_f5_cli_uses_only_frozen_defaults_and_refuses_reused_output(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    def fake_runner(path: Path, run_id: str, config: F5TransferConfig) -> pd.DataFrame:
    ...
    def fake_report(
        ...
        _config: F5TransferConfig,
    ) -> Path:
        captured["calibration_dir"] = calibration_dir
        (path / "manifest.json").write_text('{"terminal_outcome": "PASS"}', encoding="utf-8")
        memo = path / "f5-transfer-memo.md"
        memo.write_text("memo", encoding="utf-8")
        return memo

    monkeypatch.setattr(cli, "run_f5_transfer", fake_runner)
    monkeypatch.setattr(cli, "write_f5_transfer_report", fake_report)
    ...
    assert captured["config"] == F5TransferConfig()
    assert captured["calibration_dir"] == cli._CALIBRATION_DIR


def test_f5_cli_exposes_no_mutable_study_configuration() -> None:
