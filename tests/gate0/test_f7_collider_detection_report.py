"""Evidence-report contracts for the F7 collider detection study."""
...
def _retain_plot_path_without_a_graphics_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "research.gate0.f7_collider_detection_report._plot_detection_batches", write_plot
    )
            ...
            rows.append(
                {
                    ...
                    "seed_namespace": "batch-f7-collider-detection",
                    "run_id": _RUN_ID,
...
def _write_report(tmp_path: Path, records: pd.DataFrame) -> Path:
    ...
    return write_f7_collider_detection_report(
...
def test_complete_detection_evidence_passes_and_copies_boundary(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "detection" / "f7-collider-detection-summary.csv").is_file()
    assert (tmp_path / "detection" / "plots" / "f7-collider-detections.png").is_file()
...
def test_tampered_calibration_records_are_refused_before_report_output(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="SHA-256"):
        write_f7_collider_detection_report(records, output, _RUN_ID, calibration, F4LinkConfig())
...
def test_wrong_detection_identity_is_rejected(
    ...
    with pytest.raises(ValueError, match=message):
        write_f7_collider_detection_report(
            records, output, _RUN_ID, _ready_calibration(tmp_path / "calibration"), F4LinkConfig()
        )
