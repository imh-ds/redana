"""Run the one immutable F5 residual-null transfer study."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen F5 residual-null transfer.")
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F5 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F5 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        config = F5TransferConfig()
        records = run_f5_transfer(args.output_dir, run_id, config)
        memo_path = write_f5_transfer_report(
        ...
        print(f"F5 RESIDUAL-NULL TRANSFER REFUSED [{run_id or '<missing>'}]: {error}", file=sys.stderr)
        return 2

    print(
        f"F5 RESIDUAL-NULL TRANSFER [{run_id}]: {outcome}; wrote {len(records)} F5 records "
