"""Run the one immutable F1 independence null-transfer study."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen F1 independence null transfer.")
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F1 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F1 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        records = run_f1_transfer(args.output_dir, run_id, config)
        memo_path = write_f1_transfer_report(
        ...
        print(f"F1 INDEPENDENCE NULL TRANSFER REFUSED [{run_id or '<missing>'}]: {error}", file=sys.stderr)
        return 2

    print(
        f"F1 INDEPENDENCE NULL TRANSFER [{run_id}]: {outcome}; wrote {len(records)} F1 records "
