"""Run the one immutable F6 residual-null transfer study."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen F6 residual-null transfer.")
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F6 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F6 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        records = run_f6_transfer(args.output_dir, run_id, config)
        memo_path = write_f6_transfer_report(
        ...
        print(f"F6 RESIDUAL-NULL TRANSFER REFUSED [{run_id or '<missing>'}]: {error}", file=sys.stderr)
        return 2

    print(
        f"F6 RESIDUAL-NULL TRANSFER [{run_id}]: {outcome}; wrote {len(records)} F6 records "
