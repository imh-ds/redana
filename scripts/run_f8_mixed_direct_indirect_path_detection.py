"""Run the one immutable F8 mixed direct-and-indirect path detection study."""
...
from research.gate0.f8_mixed_direct_indirect_path_detection_report import (
    write_f8_mixed_direct_indirect_path_detection_report,
)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the frozen F8 mixed direct-and-indirect path detection study."
    )
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F8 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F8 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        config = F4LinkConfig()
        records = run_f8_mixed_direct_indirect_path_detection(args.output_dir, run_id, config)
        memo_path = write_f8_mixed_direct_indirect_path_detection_report(
        ...
        print(
            f"F8 MIXED DIRECT-AND-INDIRECT PATH DETECTION REFUSED [{run_id or '<missing>'}]: {error}",
            file=sys.stderr,
        )
        return 2

    print(
        f"F8 MIXED DIRECT-AND-INDIRECT PATH DETECTION [{run_id}]: {outcome}; wrote {len(records)} "
