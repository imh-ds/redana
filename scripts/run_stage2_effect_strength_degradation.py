"""Run the Stage II round 1 effect-strength degradation benchmark.

outline/plan.md section 6, scoped to exactly one of its seven named
degradation dimensions this round: effect strength, swept across three
levels on Stage I's two existing fixture shapes. Not a Gate 0 study --
no hash-pinned calibration, no per-replication artifact retention.
Reuses every Step 4 / Stage I component unchanged.
"""
...
_ALPHA = 0.05
_BASE_SEED = 20260825
_LEVELS = (("strong", 0.7), ("moderate", 0.4), ("weak", 0.2))


def run() -> dict[str, list[ConditionResult]]:
    """Run all six (shape x strength level) conditions and return them grouped by shape."""
    ...
    for shape_name, generator in (
        ...
        for level_name, coefficient in _LEVELS:
            condition_name = f"stage2-{shape_name}-{level_name}"
            fixture_fn = partial(generator, coefficient=coefficient)
...
def _print_shape(shape_name: str, conditions: list[ConditionResult]) -> None:
    print(f"== {shape_name} fixture across effect-strength levels ==")
    for (level_name, coefficient), result in zip(_LEVELS, conditions, strict=True):
        summary = result.summary
        print(f"  -- {level_name} (coefficient={coefficient}) --")
