"""Run the one immutable F2 linear direct-edge detection study."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the frozen F2 linear direct-edge detection study."
    )
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F2 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F2 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        config = F4LinkConfig()
        records = run_f2_linear_direct_edge_detection(args.output_dir, run_id, config)
        memo_path = write_f2_linear_direct_edge_detection_report(
        ...
        print(f"F2 LINEAR DIRECT-EDGE DETECTION REFUSED [{run_id or '<missing>'}]: {error}", file=sys.stderr)
        return 2

    print(
        f"F2 LINEAR DIRECT-EDGE DETECTION [{run_id}]: {outcome}; wrote {len(records)} records "
