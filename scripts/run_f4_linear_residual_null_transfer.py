"""Run the one immutable F4 linear residual-null transfer study."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the frozen F4 linear residual-null transfer.")
...
def _verify_frozen_calibration() -> None:
    ...
    if not isinstance(selection, dict) or selection.get("boundary") != _FROZEN_BOUNDARY:
        raise ValueError("calibration selection does not match the frozen F4 boundary")
    for key, expected in _FROZEN_HASHES.items():
        if calibration.get(key) != expected:
            raise ValueError(f"calibration {key} does not match frozen F4 provenance")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        records = run_f4_transfer(args.output_dir, run_id, config)
        memo_path = write_f4_transfer_report(
        ...
        print(
            f"F4 LINEAR RESIDUAL-NULL TRANSFER REFUSED [{run_id or '<missing>'}]: {error}",
            file=sys.stderr,
        )
        return 2

    print(
        f"F4 LINEAR RESIDUAL-NULL TRANSFER [{run_id}]: {outcome}; wrote {len(records)} F4 records "
